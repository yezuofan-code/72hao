...
function generateDailyArticle(products, dateStr) {
  ...
  const article = [
    `## 📱 前言`,
    ``,
    `今天要给大家评测的是 **${product.productName}**，运营商为${product.operator}，归属地${product.area}，佣金 **${product.price}元**，返佣类型为${product.backMoneyType}。`,
    ``,
    ...
    `## 💰 佣金说明`,
    ``,
    `这款产品的推广佣金为 **${product.price}元/单**，${product.backMoneyType}。`,
    `如果每天能推广 ${1 + Math.floor(seededRandom('daily-target-' + dateStr)() * 4)} 单，月收入就相当可观了。`,
    ``,
    `## ✅ 总结`,
    ``,
